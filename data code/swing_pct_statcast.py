...
def compute_all_swing_metrics(pitches: pd.DataFrame) -> dict[str, float]:
    ...
    if "description" not in pitches.columns:
        return {k: np.nan for k in ["Swing_pct", "Contact_pct", "SweetSpot_pct", "SwStr_pct", "Chase_pct"]}
    ...
    # Sweet Spot% (launch angle 8-32° on batted balls)
    if "launch_angle" in pitches.columns:
        batted_balls = pitches[pitches["launch_angle"].notna()]
        if len(batted_balls) > 0:
            sweet_spot_mask = (batted_balls["launch_angle"] >= 8) & (batted_balls["launch_angle"] <= 32)
